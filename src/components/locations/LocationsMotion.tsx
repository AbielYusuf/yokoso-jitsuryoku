...
export function LocationsMotion({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const label = root.querySelector<HTMLElement>("[data-location-label]");
      const title = root.querySelector<HTMLElement>("[data-location-title]");
      const sub = root.querySelector<HTMLElement>("[data-location-sub]");
      const items = gsap.utils.toArray<HTMLElement>("[data-location-item]");
      ...
      items.forEach((item, index) => {
        const image = item.querySelector<HTMLElement>("[data-location-image]");
        const meta = item.querySelector<HTMLElement>("[data-location-meta]");
